...
public class Truck extends Vehicle {
     ...
     *
     * @param brakePower how much power is being put into the breaks
     * @param brakeTime how long it is breaking for
     * @param airPressure how much air is being put in the tires
     */
    public void brake(
        final int brakePower, final int brakeTime, final int airPressure
    ) {
        setSpeed(
            getSpeed() - (brakePower * brakeTime) - (airPressure * brakeTime)
        );
        if (getSpeed() < 0) {
            setSpeed(0);
        }
